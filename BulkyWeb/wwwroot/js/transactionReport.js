...
function loadDataTable(startOrderDate, endOrderDate) {
    ...
    if (dataTable) {
        ...
        dataTable.ajax.url('/Admin/Report/GetTransactionReport?startOrderDate=' + startOrderDate + '&endOrderDate=' + endOrderDate).load();
        return;
    }
    // Initialize DataTable with proper AJAX URL and column definitions
    dataTable = $('#dataTableOrderReport').DataTable({
        "ajax": {
            url: '/Admin/Report/GetTransactionReport?startOrderDate=' + startOrderDate + '&endOrderDate=' + endOrderDate,
            type: "GET",
